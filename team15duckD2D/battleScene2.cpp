...
HRESULT battleScene2::init()
{
	...
	// 테스트용 삭제 될 것
	{
		IMAGEMANAGER->addImage("boss_battle", L"image/boss_battle.png", 230, 260);

		_npc = new trainerNPC;
		_npc->init("boss_battle", "비주기", 1000);
		{
			_emPms[0] = new pokemon;
			_emPms[0]->init(NULL, PM_BULBASAUR, 5, false);
			
			_emPms[1] = new pokemon;
			_emPms[1]->init(NULL, PM_CHARMANDER, 10, false);

			_emPms[2] = new pokemon;
			_emPms[2]->init(NULL, PM_SQUIRTLE, 15, false);

			_emPms[3] = new pokemon;
			_emPms[3]->init(NULL, PM_MAGIKARP, 20, false);

			_emPms[4] = new pokemon;
			_emPms[4]->init(NULL, PM_PIKACHU, 25, false);

			_emPms[5] = new pokemon;
	...
	wstring script;
	script = L"체육관장 ";
	script.append(string2wstring(_npc->getName()));
	script.append(L"(이)가\n 승부를 걸어왔다!");
...
void battleScene2::battleEvolution()
{
